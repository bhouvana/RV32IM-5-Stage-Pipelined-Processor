...
| `ICache` | `ICache.v` | `WAYS`, `CACHE_SIZE_BYTES`, `LINE_BYTES`, `XLEN`, `INIT_FILE`, `IMEM_SIZE_BYTES`, `MEM_LATENCY` (`docs/adr/0023`, `0024`) | PIPT read-only I-cache, `CACHE_MODE=CACHE_WRITEBACK_SETASSOC` only (default 4-way/4KB/16B lines); privately instantiates its own `InstructionMemory.v`, no bus port needed (nothing else shares it); `MEM_LATENCY` (default 0, bit-exact) adds real per-word wait-states to its own fill engine |
