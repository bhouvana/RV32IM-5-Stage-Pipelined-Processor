...
**Recommended next milestone**: V-4 (constrained-random testing) and V-5 (coverage) are the remaining open Phase 3 items, and now have assertions to check randomized runs against instead of only hand-computed expected values. CSR/exceptions (Phase 5 item 3) is the largest remaining ISA-extension item. FPGA readiness (Phase 7) is blocked on `DataMemory`'s combinational-read model needing a synchronous-read variant first.
