...
**Recommended next milestone**: first real hardware bring-up (the last open Phase 7 item) is the most concrete remaining target — `fpga/top.v`/`fpga/constraints_template.xdc` exist but nothing has touched a board yet. Research platform (Phase 6) and benchmarking (Phase 10) are the other open phases with no blocking dependency left standing in their way.
