...
**Status update**: RV32M (first version, single-cycle) and V-3 (assertions) are both done. **13/13 tests, 67/67 checks passing**, plus 4 embedded invariant assertions (default-on in the test runner) verified to actually catch what they're meant to.

**Recommended next milestone**: the real multi-cycle divider + generic multi-cycle-execute stall mechanism (the item `docs/adr/0006` explicitly deferred) is the largest piece of unfinished RV32M work and a genuine microarchitecture milestone in its own right -- probably the highest-value next step now that both ISA completeness and a first verification-hardening pass are done. V-4 (constrained-random testing) and V-5 (coverage) are the other open Phase 3 items, and now have assertions to check against instead of only hand-computed expected values.
